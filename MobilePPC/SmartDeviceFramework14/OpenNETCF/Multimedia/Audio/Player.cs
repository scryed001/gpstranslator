...
namespace OpenNETCF.Multimedia.Audio
{
	...
	public class Player : Audio
	{
		...
		/// </summary>
		public static int NumDevices { get { return Wave.waveOutGetNumDevs(); } }

		/// <summary>
		/// Restart a paused wave file
		/// </summary>
		public void Restart() { Wave.waveOutRestart(m_hWaveOut); }

		/// <summary>
		/// Pause Play
		/// </summary>
		public void Pause() { Wave.waveOutPause(m_hWaveOut); }
		...
		public int Volume
		{
			...
			{
				int vol = 0;
				CheckWaveError(Wave.waveOutGetVolume(m_hWaveOut, ref vol));
				return vol;
			}
			set
			{
				CheckWaveError(Wave.waveOutSetVolume(m_hWaveOut, Convert.ToInt32(value)));
			}
		}

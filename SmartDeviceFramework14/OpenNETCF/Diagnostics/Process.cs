...
namespace OpenNETCF.Diagnostics
{
	...
	public class Process : Component, IDisposable
	{
		...
		public void Kill()
		{
			if(!m_exited)
			{
				...
				if(this.Handle != IntPtr.Zero)
				{
					...
					{
						//close the handle
						if(CloseHandle(this.m_pinfo.hProcess) == 0)
						{
							this.m_pinfo.hProcess = IntPtr.Zero;
						}
		...
		public bool WaitForExit(int milliseconds)
		{
			...
			int result = OpenNETCF.Threading.NativeMethods.WaitForSingleObject(this.m_pinfo.hProcess, wait);
		...
		public IntPtr Handle
		{
			get
			{
				...
				if(m_pinfo.hProcess == IntPtr.Zero)
				{
					...
					{
						//populate handle
						m_pinfo.hProcess = OpenProcess(0, false, m_pinfo.dwProcessID);
					}
					else
					{
		...
		[Obsolete("Use GetCurrentProcess to return a Process object for the active process", false)]
		...
		internal extern static IntPtr OpenProcess(uint fdwAccess, bool fInherit, uint IDProcess);

		[DllImport("coredll.dll", EntryPoint="OpenProcess", SetLastError=true)]
		internal extern static IntPtr OpenProcess(uint fdwAccess, bool fInherit, int IDProcess);
...
//		private extern static int GetCurrentProcessId();
		...
		private sealed class ShellExecuteInfo
		{
			...
			public UInt32 fMask			= 0x00000440; 

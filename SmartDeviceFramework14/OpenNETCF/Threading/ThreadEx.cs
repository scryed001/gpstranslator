...
namespace OpenNETCF.Threading
{
	...
	public class ThreadEx
	{
		...
		{
			get { return NativeMethods.CeGetThreadPriority(m_handle); }
			set
			{
				...
				if(!NativeMethods.CeSetThreadPriority(m_handle, value))
				{
					throw new OpenNETCF.Win32.WinAPIException("Failed to set thread priority");
				}

				throw new BestPracticeViolationException("This property is for real-time threads.  Do not use it for application threads");
			}
		}
		...
		public ThreadEx(System.Threading.ThreadStart start)
		{
			...
			m_terminationEvent = NativeMethods.CreateEvent(IntPtr.Zero, true, false, OpenNETCF.GuidEx.NewGuid().ToString());
		...
		public void Suspend()
		{
			...
			lock(this)
			{
				NativeMethods.SuspendThread(m_handle);
			}
		...
		public void Resume()
		{
			...
			lock(this)
			{
				NativeMethods.ResumeThread(m_handle);
			}
			m_state = ThreadState.Running;
		}
		...
		{	
			lock(this)
			{
				...
				NativeMethods.TerminateThread(m_handle, 0);

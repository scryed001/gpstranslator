...
namespace OpenNETCF.Web.Services2
{
	public class WoTe
	{
		...
		public static void FindCryptoWoteInfo()
		{
			try	
			{
				OpenNETCF.Security.Cryptography.NativeMethods.ProviderInfo [] pia = OpenNETCF.Security.Cryptography.NativeMethods.Prov.EnumProviders();
				isCryptoApi = true;
				foreach(OpenNETCF.Security.Cryptography.NativeMethods.ProviderInfo pi in pia)
				{
					if(pi.name == OpenNETCF.Security.Cryptography.NativeMethods.ProvName.MS_ENHANCED_PROV)
						isEnhanced = true;
					if(pi.type == OpenNETCF.Security.Cryptography.NativeMethods.ProvType.DSS_DH)
						isDsa = true;
				}
